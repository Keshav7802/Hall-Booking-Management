    ...
    const fetchEventDetails = async () => {
      const bookingsWithEventDetails = await Promise.all(
        bookingData.map(async (booking) => {
          ...
          const eventResponse = await fetch("http://localhost:3001/event/getEvent/${booking.eventName}", 
          {
            method: "GET",
            headers: {
              "Content-Type": "application/json",
          ...
          // console.log({ ...booking, eventDetails });
      ...
      const combinedData = await Promise.all(
        bookingData.map(async (booking) => {
          ...
          const hallNames = await Promise.all(
            booking.hallIDs.map(async (hall) => {
              const hallResponse = await fetch("http://localhost:3001/hall/getHall/${hall}", 
              {
                method: "GET",
                headers: {
                  "Content-Type": "application/json",
              ...
              <li className="p-2">
                ...
                >
                  <h5 className="mb-2 text-xl font-bold tracking-tight">
                    ...
                    TO{" "}
                    {booking.eventDetails && booking.eventDetails.endDateTimeDateTime ? (new Date(booking.endDateTime).toLocaleTimeString(
                      "en-US",
                      timeOptions
                    )) : ("No end time")}{" "}
                  </h5>
                  <div className="flex justify-between items-end">
                    <div className="font-normal text-black text-sm">
                      <div>Affiliated Department/Club: {booking.eventDetails && booking.eventDetails.clubID ? (booking.eventDetails.clubID) :("No associated club")}</div>
                      <div>Reason : {booking.eventDetails && booking.eventDetails.eventPurpose ? (booking.eventDetails.eventPurpose) : ("No purpose")}</div>
                    </div>
